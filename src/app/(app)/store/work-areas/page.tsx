...
import WorkAreasPage from "@/features/store/components/WorkAreasPage";
import { listWorkAreas } from "@/features/store/server/workforce-service";
...
export default async function Page({ searchParams }: { searchParams?: { storeId?: string } }) {
  ...
  const initialAreas = await listWorkAreas({ user, storeId });
  return <WorkAreasPage storeId={storeId} readOnly={user.role === "admin"} initialAreas={initialAreas} />;
}
