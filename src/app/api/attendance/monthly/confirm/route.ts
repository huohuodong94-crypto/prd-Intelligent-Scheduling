...
import { confirmMonthlyAttendance, MonthlyAttendanceServiceError } from "@/features/attendance/server/monthly-attendance-service";
import { fail, ok, readJson } from "@/lib/api";
import { requireSession } from "@/lib/auth";
import { requireStoreAccess } from "@/lib/authorization";
import { monthlyConfirmSchema } from "@/lib/contracts/monthly-attendance";

export async function POST(req: Request) {
  const auth = await requireSession(["manager"]);
  if ("error" in auth) return fail(auth.error, auth.status);
  try {
    const input = monthlyConfirmSchema.parse(await readJson(req));
    const access = await requireStoreAccess(["manager"], input.storeId);
    if ("error" in access) return fail(access.error, access.status);
    return ok(await confirmMonthlyAttendance(access.scope, input));
  } catch (error) {
