...
import { getSchedulingReport, ReportServiceError } from "@/features/reports/server/report-service";
import { fail, ok } from "@/lib/api";
import { requireSession } from "@/lib/auth";
import { requireStoreAccess } from "@/lib/authorization";
import { schedulingReportQuerySchema } from "@/lib/contracts/reports";
...
export async function GET(request: Request) {
  ...
  try {
    const query = schedulingReportQuerySchema.parse(Object.fromEntries(new URL(request.url).searchParams));
    ...
    return ok(await getSchedulingReport(access.scope, query.weekOf));
  } catch (error) {
